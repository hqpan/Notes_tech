...
- B 树：相较于 B+ 树，去除了搜索码中的冗余值，但增加了整体复杂性，实际应用中 B+ 树索引更为常用；
- B+ 树的结构：balanced，一种平衡树结构，也是一种多级索引结构；
  ...
  - 优点：插入、修改、查找效率较高；
- B+ 树查询节点的时间复杂度：$O(log_{\frac{n}{2}}N)$；
  - $n$：表示节点所含指针数量；
  - $N$：表示文件中搜索码的总数；
...
- 顺序索引和散列索引：
  - 顺序索引：必须访问索引结构，从而定位数据，需要大量的 I/O 操作；
  - 散列索引：避免定位数据时访问索引结构；
- 桶：存储多条记录的存储单位；
- 桶溢出：
  - 引发桶溢出的因素：
  ...
  - 解决方案：
    - 预设桶容量时，预留 20% 的闲置空间；
    - 为即将溢出的桶提供一个溢出桶，将给定桶的所有溢出桶用链表连接；
...
- ==面试题== 简述乐观锁和悲观锁？
  ...
  - 乐观锁：乐观锁认为其它进程不会同时修改数据，因此不对数据上锁；
    - 在执行更新时判断在此期间其它进程是否修改了数据；
    - 若其它进程已修改数据，则放弃操作；
  - 悲观锁：悲观锁认为其它进程会同时修改数据，因此需要对数据上锁，操作完成后释放锁；
