...
- 卷积的类型：
  - full：参见网络博客上的示意图；
  - same：使用 Padding，输入输出的图片大小相同；
  - valid：仅当过滤器完全落入图片像素中，才能开始计算；
...
- Inception 层：其输出中叠加了各种不同类型过滤器和池化层的输出，根据参数自行学习，选取合适的过滤器；
  - 优点：代替人工来确定卷积层中的过滤器类型、是否需要创建卷积层、池化层；
  - 缺点：结构复杂，计算量较大；
    - 解决方案：使用$1\times 1$过滤器，构建瓶颈层，使得 channels 减少，从而减小计算量；
- GoogleLeNet：一种 Inception 网络，由 Google 公司的员工研发；

## 13.4 迁移学习

- 迁移学习：
  - 使用他人的网络结构及训练好的参数，用自行设计的 Softmax 层替换对方的 Softmax 层；
  - 可选择冻结特定层的参数；
  - 若自己的训练集规模越大，则可不冻结的层数就越多；
  - 极端情况下，可不冻结所有层的参数，仅使用他人的参数作为初始化参数，然后使用梯度下降，在自己的数据集上训练参数；

# 14. 杂谈

- 表征学习：是机器学习中的一类方法，用于自动找出适合表示数据的方式，而深度学习是具有多级表示的表征学习方法，可以逐级表示越来越抽象的概念或模式；
- 当数据规模较小时，使用手工构建的特征有助于提高精度；
- 提升模型效果的方法：
  - 使用多个网络的输出结果，将其做平均处理；
...
- 当前进度：CS 224n，Chapter 0 已完成；
  - 每周 1 天学习 1 Chapter 的课程，剩余 6 天学习纸质教材；
- 在理论学习期间择机阅读：
