...
- TCP 感知网络拥塞的方式：即感知丢包的方式；
  ...
  - 某个特定的 datagram 对应的 ACK 超时；
  - 收到3个冗余的 ACK（即一个初始 ACK 和3个冗余 ACK）；
    - 若收到2个冗余的 ACK，则可能是报文乱序；
    - 若收到3个冗余的 ACK，则可能是 datagram 丢失；
    - 若收到4个冗余的 ACK，则更可能是 datagram 丢失；
    - 因此在收到3个冗余的 ACK 后，可初步判定为 datagram 丢失，即发生网络拥塞；
  - 注意：
    - 超时：表明网络拥塞程度较重；
    - 收到3个冗余 ACK：表明网络还能传输部分 datagram；
...
- 快速恢复：
  ...
  - 将拥塞窗口值加3，重传丢失的 datagram；
    - 收到3个冗余的 ACK，表明有3个 datagram 已到达接收方的缓存中，不再占用网络资源，因此可将拥塞窗口加3个 MSS；
  - 若发送方继续收到冗余的 ACK，则每收到一个冗余的 ACK，即将拥塞窗口加1个 MSS；
  - 当发送方接收到新 datagram 对应的 ACK 时，发送端将拥塞窗口降低为步骤一中的值，此后进入拥塞避免状态；
...
- ==面试题== 简述拥塞控制模式？
  ...
  - 若发生超时：
    ...
    - 令拥塞窗口大小为1 MSS；
    - 发送方每收到一个 datagram 的 ACK，即将拥塞窗口增加1个 MSS（拥塞窗口大小指数增长）；
