...
- 递归：
  - 递归三要素：边界条件、递归前进段、递归返回段；
    ...
    - 当边界条件满足时，递归返回；
  - 递归算法适用的场景：
    - 数据的定义是按递归定义的，E.g. Fibonacci函数；
    - 问题解法按递归算法实现：问题本身没有明显的递归结构，但用递归求解比迭代求解更简单，E.g. Hanoi问题；
...
- 红黑树的性质：
  ...
  - 其它操作的运行时间均为对数级别；
-   备注：红黑树的插入、读取方法已掌握，删除方法仅理解基本原理，未实现；
...
- 基于拉链法的散列表：
  ...
  - 缺点：
    - 散列后键的顺序信息丢失，因此查找最大最小键的时间复杂度为线性级别；
- 动态调整数组大小：若能预估数组大小，则无需动态调整；
  - 调整目标：保持链表的平均长度在2-8之间；
...
|       数据结构       |      具体实现      |                      优点                      |                  缺点                  |
| :------------------: | :----------------: | :--------------------------------------------: | :------------------------------------: |
| 无序链表中的顺序查找 | SequentialSearchST |                 适用于小型问题                 |           处理大型符号表较慢           |
| 有序数组中的二分查找 |   BinarySearchST   | 查找效率高，空间需求低，能进行有序性相关的操作 |               插入操作慢               |
...
|     算法（数据结构）     | 最坏情况下查找 | 最坏情况下插入 | 平均情况下查找 | 平均情况下插入 | 是否支持有序性操作 |
| :----------------------: | :------------: | :------------: | :------------: | :------------: | :----------------: |
|   顺序查询（无序链表）   |       N        |       N        | $\frac{N}{2}$  |       N        |         否         |
|   二分查找（有序数组）   |      lgN       |       N        |      lgN       | $\frac{N}{2}$  |         是         |
| 二叉树查找（二叉查找树） |       N        |       N        |    1.39lgN     |    1.39lgN     |         是         |
|     2-3树（红黑树）      |      2lgN      |      2lgN      |    1.00lgN     |    1.00lgN     |         是         |

- 基于拉链法的散列表：
  - 若键的数量为N，链表数组的长度为M，则未命中查找和插入操作所需的比较次数均为$\frac{N}{M}$；
- 散列表不支持有序性相关操作；
