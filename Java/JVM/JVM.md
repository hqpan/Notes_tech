...
- 空间分配担保：在执行 Minor GC 之前，须检查老年代最大可用的连续空间是否大于新生代所有对象总空间；
  - 若该条件成立，则 Minor GC 操作是安全的；
  - 若该条件不成立，则 JVM 将根据设置值判断是否允许担保失败；
    - `-XX：HandlePromotionFailure`：设置是否允许担保失败；
    - 若允许担保失败，则检测老年代最大可用的连续空间是否大于历次晋升到老年代对象的平均大小；
      - 若大于，则尝试进行一次 Minor GC，若失败，则 Full GC；
      - 若小于，则 Full GC；
    - 若不允许担保失败，则Full GC；
  - 注意：
    - 若老年代的最大连续空间大于新生代对象总大小或历次晋升到老年代对象的平均大小，则执行 Minor GC，否则执行 Full GC；
    - JVM 不再使用`-XX：HandlePromotionFailure`；
