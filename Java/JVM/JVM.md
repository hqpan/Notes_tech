...
- 验证的目的：确保 Class 文件的字节流中包含的信息符合《Java虚拟机规范》的约束要求；
- 验证的内容：
...
- 准备：在方法区中，为类变量（即被 static 修饰的静态变量）分配内存并设置类变量初始值；
...
- 自初始化阶段起，JVM 才真正开始执行类中编写的 Java 程序；
- 在静态语句块中，只能访问到定义在静态语句块之前的变量，对于定义于其后的变量，仅能被此前的静态语句块赋值，但是不能被访问；
- 准备阶段和初始化阶段的区别：
  - 准备阶段：将变量值赋为“零值”；
  - 初始化阶段：将变量值赋值为程序员设定的值；
...
- 在虚拟机加载Class文件的时候进行动态连接（具体见第7章）。也就是说，在Class文件中不会保存各个方法、字段最终在内存中的布局信息，这些字段、方法的符号引用不经过虚拟机在运行期转换的话是无法得到真正的内存入口地址，也就无法直接被虚拟机使用的。当虚拟机做类加载时，将会从常量池获得对应的符号引用，再在类创建时或运行时解析、翻译到具体的内存地址之中。
