...
- ==面试题== Overloading 和 Override 的区别？
  ...
  - Override：在不同子类中有不同的实现；
    ...
    - 覆写的方法访问权限不得小于被覆写的方法，访问权限由高至低为`public，protected, default，private`；
...
- 自动装箱和自动拆箱：基本数据类型、包装类型之间相互转换，包装类可直接参与数值计算；
- 抽象类`Number`中的方法：
...
- 缓存池：
  - `new Integer(123)`和`Integer.valueOf(123)`的区别？
    - 前者：每次执行时都将创建一个新对象；
    - 后者：使用缓存池中的对象，多次调用将取得同一对象的引用；
  - `valueOf()`：若值在缓存池中，则返回缓存池中的内容；
  - Integer 缓存池的默认大小为$[-128,127]$；
